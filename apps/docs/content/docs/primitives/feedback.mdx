...
`Feedback` pairs a trigger element with animated contextual messaging. Render `Feedback.Content` inside `Feedback.Root`, and control visibility with the `show` prop.

## Examples

### Chained Messages

Use `contentKey` when the message changes while feedback is visible so the content transition replays cleanly.

<ComponentExample name="feedback/chained" />
