...
const RankedListContext = React.createContext<RankedListContextValue<unknown> | null>(null)
...
const GroupContext = React.createContext<GroupContextValue<unknown> | null>(null)
...
const GroupItemContext = React.createContext<GroupItemContextValue<unknown> | null>(null)
...
function RankedListInner<T>({
  ...
  const emptyChild = childArray.find(child =>
    React.isValidElement(child) && child.type === Empty
  )
...
function GroupItem({ children, className }: GroupItemProps): React.ReactElement {
...
interface GroupItemIndexProps {
  children?: (context: { globalIndex: number; rank: number; groupIndex: number }) => React.ReactNode
  className?: string
}

function GroupItemIndex({ children, className }: GroupItemIndexProps): React.ReactElement | null {
...
function GroupItemValue<T>({ children }: GroupItemValueProps<T>): React.ReactElement | null {
  const groupItemContext = useGroupItem<T>()
  return <>{children(groupItemContext.item, groupItemContext)}</>
}

interface GroupDividerProps {
  children?: (context: { label: string; groupIndex: number }) => React.ReactNode
  className?: string
}

function GroupDivider({ children, className }: GroupDividerProps): React.ReactElement | null {
